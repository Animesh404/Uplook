...
from app.db.models import Content, User, UserGoal, Goal, ActivityLog, JournalEntry, MoodLog, CategoryEnum, ContentTypeEnum
from app.db.schemas import Recommendation
...
class RecommendationService:
    ...
    def generate_recommendations(self, db: Session, user_id: int) -> List[Dict[str, Any]]:
        ...
        goal_categories = [self.category_mapping.get(goal.goal.name, CategoryEnum.SELF_CONFIDENCE) for goal in user_goals]
        ...
        if wellness_score < 50:
            recommendations.extend(self._get_urgent_recommendations(db, user_id, goal_categories, completed_content_ids))
        
        # Priority 2: Goal-based recommendations
        recommendations.extend(self._get_goal_based_recommendations(db, goal_categories, completed_content_ids))
        
        # Priority 3: Trending/popular content
        recommendations.extend(self._get_popular_recommendations(db, completed_content_ids))
        
        # Priority 4: Exploration recommendations
        recommendations.extend(self._get_exploration_recommendations(db, completed_content_ids))
    ...
    def _get_urgent_recommendations(self, db: Session, user_id: int, goal_categories: List[CategoryEnum], completed_ids: List[int]) -> List[Dict[str, Any]]:
        ...
        if mood_trends and any(t['mood_score'] < 0.4 for t in mood_trends[-3:]):
            stress_content = db.query(Content).filter(
                and_(
                    Content.category == CategoryEnum.ANXIETY,
                    Content.content_type.in_([ContentTypeEnum.MEDITATION, ContentTypeEnum.MUSIC]),
            ...
            for content in stress_content:
                recommendations.append({
                    'content': content,
                    'reason': 'Your recent biometric data suggests elevated stress levels',
                    'priority': 1
                })
        ...
        if sentiment_trends:
            ...
            if any(any(keyword in entry.entry_text.lower() for keyword in sleep_keywords) for entry in recent_entries):
                sleep_content = db.query(Content).filter(
                    and_(
                ...
                for content in sleep_content:
                    recommendations.append({
                        'content': content,
                        'reason': 'Your journal entries suggest sleep-related concerns',
                        'priority': 1
                    })
        
        return recommendations
    
    def _get_goal_based_recommendations(self, db: Session, goal_categories: List[CategoryEnum], completed_ids: List[int]) -> List[Dict[str, Any]]:
        ...
        for category in goal_categories:
            ...
            for content in content_items:
                recommendations.append({
                    'content': content,
                    'reason': f'Recommended based on your goal: {category.value.replace("_", " ").title()}',
                    'priority': 2
                })
        
        return recommendations
    
    def _get_popular_recommendations(self, db: Session, completed_ids: List[int]) -> List[Dict[str, Any]]:
        """Get popular content recommendations"""
        # Get most completed content in the last 30 days
        popular_content = db.query(Content).join(ActivityLog).filter(
            and_(
                ActivityLog.completed_at >= datetime.now() - timedelta(days=30),
                ~Content.id.in_(completed_ids)
            )
        ).group_by(Content.id).order_by(func.count(ActivityLog.id).desc()).limit(3).all()
    ...
    def _get_exploration_recommendations(self, db: Session, completed_ids: List[int]) -> List[Dict[str, Any]]:
        ...
        for category in all_categories[:2]:  # Pick 2 random categories
            ...
            for content in content_items:
                recommendations.append({
                    'content': content,
                    'reason': f'Explore {category.value.replace("_", " ").title()} content',
                    'priority': 4
                })
    ...
    def _get_next_milestone(self, completed_activities: int) -> str:
        ...
        if completed_activities < 5:
            return f"Complete {5 - completed_activities} more activities to reach your weekly goal!"
        elif completed_activities < 10:
            return f"Great progress! {10 - completed_activities} more activities to achieve your stretch goal!"
        else:
