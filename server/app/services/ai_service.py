...
from app.db.models import JournalEntry, MoodLog, User
...
class AIService:
    ...
    def get_sentiment_trends(self, db: Session, user_id: int, days: int = 30) -> List[Dict[str, Any]]:
    ...
    def get_mood_trends(self, db: Session, user_id: int, days: int = 30) -> List[Dict[str, Any]]:
    ...
    def generate_insights(self, db: Session, user_id: int) -> List[str]:
        ...
        if sentiment_trends:
            avg_sentiment = np.mean([t['sentiment_score'] for t in sentiment_trends])
            if avg_sentiment > 0.3:
                insights.append("Your journal entries show a positive emotional trend this week!")
            elif avg_sentiment < -0.3:
                insights.append("Your recent journal entries suggest some challenges. Consider focusing on stress-reduction activities.")
            else:
                insights.append("Your emotional state appears balanced based on your journal entries.")
        
        if mood_trends:
            avg_mood = np.mean([t['mood_score'] for t in mood_trends])
            if avg_mood > 0.7:
                insights.append("Your wearable data indicates excellent overall wellness!")
            elif avg_mood < 0.4:
                insights.append("Your biometric data suggests elevated stress levels. Consider meditation or relaxation exercises.")
            else:
                insights.append("Your biometric indicators show room for improvement in stress management.")
        
        if not insights:
            insights.append("Keep logging your activities to get personalized insights!")
    ...
    def calculate_wellness_score(self, db: Session, user_id: int) -> Dict[str, Any]:
        ...
        if sentiment_trends:
            sentiment_score = (np.mean([t['sentiment_score'] for t in sentiment_trends]) + 1) / 2
